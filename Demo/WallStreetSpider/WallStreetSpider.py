...
def ContentSave(item):
    ...
    type = item[0]
    print(item[1])
    # content = item[1].decode("unicode")
    # json格式数据中，需从'\\uxxxx'形式的unicode_escape编码
    # 转换成u'\uxxxx'的unicode编码
    content = item[1].encode("utf-8")
    # print content
    # district的筛选
    categorySet = item[2]
...
def func( page ):
    ...
    content = Spider(url, data).decode("utf-8")
